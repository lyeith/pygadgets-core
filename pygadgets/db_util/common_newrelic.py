...
def exec_sql(conn, query, param=None, dct=True, fetch=True, page=False):
    ...
    if type(conn) in (pymysql.connections.Connection,
                      newrelic.hooks.database_mysqldb.ConnectionWrapper) and dct:
        cur = conn.cursor(pymysql.cursors.DictCursor)
    elif type(conn) in (psycopg2.extensions.connection,
                        newrelic.hooks.database_psycopg2.ConnectionWrapper) and dct:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    elif type(conn) in (sqlite3.Connection, newrelic.hooks.database_sqlite.ConnectionWrapper) and dct:
